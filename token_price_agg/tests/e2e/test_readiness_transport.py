...
def test_readiness_endpoint_returns_503_when_provider_transport_is_unhealthy(
    ...
    monkeypatch.setattr(
        "token_price_agg.providers.registry.ProviderRegistry.transport_unhealthy",
        lambda _: True,
    )
...
def test_readiness_endpoint_returns_503_when_close_wait_threshold_is_exceeded(
    ...
    monkeypatch.setattr(
        "token_price_agg.api.routes.ready.count_process_close_wait_sockets",
        lambda: 3,
    )
    monkeypatch.setattr(
        "token_price_agg.providers.registry.ProviderRegistry.recycle_transports",
        _recycle_transports,
    )
